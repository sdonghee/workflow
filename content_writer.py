...
def _build_system_prompt(category: str) -> str:
    ...
    persona = {
        "여행_항공_호텔": (
            ...
            f"'{core_kw}' 분야에 특화된 매력적인 생활 정보 블로그 콘텐츠 기획·제작 AI입니다. "
            "월 1,000만 PV를 달성한 여행 블로거로서, "
            "독자가 읽고 바로 행동에 옮길 수 있는 압도적으로 유용한 포스트를 작성합니다."
        ),
        "정부혜택": (
            "당신은 사람들의 시선을 사로잡는 마케팅 전문가이자, "
            f"'{core_kw}' 분야에 특화된 생활 정보 블로그 콘텐츠 기획·제작 AI입니다. "
            "복잡한 정부 정보를 누구나 이해하도록 쉽게 풀어내고, "
            "독자가 신청 버튼을 바로 누르도록 강력하게 행동을 유도합니다."
        ),
        "건강": (
            "당신은 사람들의 시선을 사로잡는 마케팅 전문가이자, "
            f"'{core_kw}' 분야에 특화된 헬스케어 블로그 콘텐츠 기획·제작 AI입니다. "
            "의학·과학 정보를 신뢰감 있게 전달하되, "
    ...
    base = persona.get(
        category,
        f"당신은 '{core_kw}' 분야에 특화된 블로그 콘텐츠 기획·제작 전문가입니다.",
    )
    ...
    return (
        ...
        "3. ★★ 하나의 주제에만 집중 ★★ — 여러 주제를 뒤섞지 말 것\n"
        "   예) '항공권 할인법'이면 오직 항공권 할인법만 / '여행지 추천'이면 오직 여행지만\n"
        "4. 독자와 대화하는 친근한 구어체\n"
        "5. 독자가 오늘 바로 써먹을 수 있는 실용 정보 중심\n"
        "6. 네이버 블로그 HTML 렌더링 규칙 완벽 준수 (인라인 style만, script/class 금지)"
    )
